...
const updateUserProfile = async (req, res) => {
  try {
    ...
    if (req.file) {
      const profileImage = `${domain}/uploads/profiles/${req.file.filename}`; //file ko orginal name lineko lagi .filename jun chai mw bata leko. 
...
const getUserProfile = async (req, res) => {
  try {
    const profile = await UserProfiles.findOne({ user: req.user._id })
    .populate(
      "user",
      ["name", "email"]
    );
